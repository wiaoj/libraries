...
//Console.WriteLine("=== 1. BAŞARILI SENARYO (Doğru Kullanım) ===");
//HappyPath_CorrectUsage();

//Console.WriteLine("\n=== 2. RUNTIME (ÇALIŞMA ZAMANI) FELAKETİ (Memory Leak) ===");
//RuntimeDisaster_ForgettingDispose();

//Console.WriteLine("\n=== 3. DERLEYİCİ HATALARI (Aşağıdaki metotları incele) ===");
//await Correct_AsyncAwaitTest();
//// CompilerError_AsyncAwaitTest(); // Yorumu kaldırırsan derlenmez!
//// CompilerError_ClassPropertyTest(); // Yorumu kaldırırsan derlenmez!

//Console.ReadLine();


//#region 1. DOĞRU VE GÜVENLİ KULLANIM (Happy Path)

//static void HappyPath_CorrectUsage() {
//    // 1. Stack (Yığıt) belleği ayırıyoruz. (GC'nin haberi yok, sıfır maliyet)
//    Span<int> stackSpace = stackalloc int[128];

//    // 2. 'using' anahtar kelimesi HAYATİDİR. Metot bittiğinde otomatik Dispose() çağrılır.
//    using ValueList<int> list = new(stackSpace);

//    // 3. Veri ekleyelim. 128'i geçerse sessizce ArrayPool'a geçecek.
//    for(int i = 0; i < 150; i++) {
//        list.Add(i * 2);
//    }

//    Console.WriteLine($"Liste dolduruldu. Eleman Sayısı: {list.Count}, Kapasite: {list.Capacity}");

//    // 4. Allocation yapmayan SelectLazy ile okuma yapıyoruz.
//    Console.WriteLine("İlk 5 eleman (Sıfır çöp ile LINQ benzeri):");
//    int counter = 0;
//    foreach(int item in list.SelectLazy(x => x + 100)) {
//        if(counter++ >= 5) break;
//        Console.Write($"{item} ");
//    }
//    Console.WriteLine();

//    // Metot burada bitiyor. 'using' sayesinde varsa ArrayPool iade edildi, Stack silindi.
//}

//#endregion

//#region 2. ÇALIŞMA ZAMANI (RUNTIME) FELAKETİ: MEMORY LEAK

//static void RuntimeDisaster_ForgettingDispose() {
//    Console.WriteLine("UYARI: Dispose edilmeyen listeler ArrayPool'u tüketiyor...");

//    Span<int> stackSpace = stackalloc int[64];

//    // Düşün ki bu metot bir Web API endpoint'i ve saniyede 1000 kez çağrılıyor.
//    for(int i = 0; i < 10000; i++) {
//        // Bilerek using YAZMADIK. 
//        ValueList<int> list = new(stackSpace);

//        // 64 kapasiteyi bilerek aşıyoruz. Sistem ArrayPool'dan dizi (Heap) KİRALAYACAK.
//        for(int j = 0; j < 5000; j++) {
//            list.Add(j);
//        }

//        // BURASI ÇOK KRİTİK:
//        //list.Dispose(); //çağırmadığımız için kiraladığımız 5000'lik dizi havuza GERİ DÖNMEDİ!
//        // Döngü her çalıştığında ArrayPool "Elimde dizi kalmadı" deyip yeni dizi yaratmak zorunda kalacak.
//        // Sonuç: Muazzam bir Memory Leak (Bellek sızıntısı) ve uygulamanın çökmesi!
//    }
//    Console.WriteLine("Döngü bitti ama ArrayPool içindeki yüzlerce dizi çöpe (GC'ye) terk edildi!");
//}

//#endregion

//#region 3. DERLEYİCİ HATALARI (ASYNC, CLASS, BOXING)

//// !!! TEST ETMEK İÇİN AŞAĞIDAKİ METODUN YORUM SATIRLARINI KALDIR !!!

////static async Task CompilerError_AsyncAwaitTest() {
////    Span<int> stackSpace = stackalloc int[64];
////    ValueList<int> list = new ValueList<int>(stackSpace);
////    list.Add(1);

////    // HATA 1: CS4008 - 'ref struct'lar async metotlarda await sınırını GEÇEMEZ.
////    // NEDENİ: Await çağrıldığında mevcut Thread işi bırakır, state makinesi Heap'e kopyalanır.
////    // Ama 'list' struct'ımız Stack'te (stackalloc) duruyor. Heap'e taşınamaz!

////    await Task.Delay(1000);

////    // Await'ten sonra list'i kullanmaya çalışırsan sistem çökerdi, Microsoft derleyiciyle engelliyor.
////    Console.WriteLine(list[0]);
////}
//static async Task Correct_AsyncAwaitTest() {
//    int result = 0; // Heap'e kaçmasında sorun olmayan normal bir değişken

//    // 1. KAPSAM (SCOPE) AÇIYORUZ
//    {
//        Span<int> stackSpace = stackalloc int[64];
//        using ValueList<int> list = new ValueList<int>(stackSpace); // using var!

//        list.AddRange(42,43,44,45,46);

//        // İşimi bitirdim, veriyi güvenli bir değişkene alıyorum.
//        result = list[0];

//    } // 2. KAPSAM KAPANDI! 
//      // 'using' list.Dispose()'u çağırdı. list Stack'ten silindi!

//    // 3. AWAIT ZAMANI
//    // Derleyici artık rahat, çünkü 'await' sınırından karşıya geçmeye çalışan
//    // bir 'ref struct' kalmadı. Sadece zavallı 'result' değişkeni var.
//    await Task.Delay(1000);

//    Console.WriteLine($"Sonuç: {result}");
//}

//// !!! TEST ETMEK İÇİN AŞAĞIDAKİ SINIFIN YORUM SATIRLARINI KALDIR !!!

////public class UcubeSınıf {
////    // HATA 2: CS8345 - ref struct'lar normal bir sınıfın property'si/field'ı OLAMAZ!
////    // NEDENİ: Sınıflar Heap'te (Yığın) yaşar. ValueList ise "Ben sadece Stack'te (Yığıt) yaşarım" 
////    // kuralına (ref struct) sahip. İkisi bir araya gelemez.

////    public ValueList<int> SuperHizliListem { get; set; }

////    // HATA 3: CS0029 - Kutuya Koyma (Boxing) yapılamaz.
////    // NEDENİ: Object/Interface Heap'tir. ref struct Heap'e kaçamaz.
////    public object BoxIt() {
////        Span<int> stackSpace = stackalloc int[10];
////        ValueList<int> list = new ValueList<int>(stackSpace);

////        return (object)list; // Cast edilemez!
////    }
////}
