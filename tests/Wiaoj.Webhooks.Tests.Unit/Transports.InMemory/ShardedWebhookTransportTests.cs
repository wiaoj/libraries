...
public sealed class ShardedWebhookTransportTests {
    ...
    public sealed class TheConstructorAndDisposal {
        ...
        public void Dispose_CascadesDisposal_ToAllUnderlyingShards() {
            ...
            // Assert: Channels in underlying shards must be completed/closed
    ...
    public sealed class TheRoutingBehavior {
        ...
        public async Task EnqueueAsync_WithPowerOfTwoShards_UsesBitmask_AndRoutesConsistently() {
            ...
            WebhookDeliveryJob job1 = new(WebhookTestFactory.CreateEndpointId(), partitionKey, "order.created", WebhookTestFactory.CreateEvent());
            WebhookDeliveryJob job2 = new(WebhookTestFactory.CreateEndpointId(), partitionKey, "order.paid", WebhookTestFactory.CreateEvent());
            ...
            int expectedShardIndex = (int)(hash & 7ul); // 8 shard mask = 7

            InMemoryWebhookTransport targetShard = (InMemoryWebhookTransport)sharded.GetShard(expectedShardIndex);

            // Assert: Strict FIFO sequence inside target shard
        ...
        public async Task EnqueueAsync_WithNonPowerOfTwoShards_UsesModulo_AndRoutesConsistently() {
            ...
            WebhookDeliveryJob job1 = new(WebhookTestFactory.CreateEndpointId(), partitionKey, "invoice.created", WebhookTestFactory.CreateEvent());
            WebhookDeliveryJob job2 = new(WebhookTestFactory.CreateEndpointId(), partitionKey, "invoice.finalized", WebhookTestFactory.CreateEvent());
        ...
        public async Task EnqueueAsync_IsDeterministic_AcrossSeparateTransportInstances() {
            ...
            WebhookDeliveryJob job = new(WebhookTestFactory.CreateEndpointId(), partitionKey, "test.event", WebhookTestFactory.CreateEvent());

            // Act
            await routerInstance1.EnqueueAsync(job);
            await routerInstance2.EnqueueAsync(job);

            // Find which shard received the job in instance 1 and instance 2
            ...
            // Assert: Both instances must pick the exact same shard index (Proving cross-process determinism)
        ...
        public async Task EnqueueAsync_HandlesUnicodeAndSpecialCharacters_WithoutFailing() {
            ...
            foreach(string key in complexKeys) {
                WebhookDeliveryJob job = new(WebhookTestFactory.CreateEndpointId(), key, "unicode.test", WebhookTestFactory.CreateEvent());
        ...
        public async Task EnqueueAsync_HandlesExtremelyLongPartitionKeys_WithoutCrashing() {
            ...
            // 10,000 character partition key
            string longKey = new('x', 10_000);
            WebhookDeliveryJob job = new(WebhookTestFactory.CreateEndpointId(), longKey, "long.key.test", WebhookTestFactory.CreateEvent());
    ...
    public sealed class TheDelayedScheduling {
        ...
        public async Task EnqueueAsync_WithDelay_ForwardsDelayToCorrectShard() {
            ...
            WebhookDeliveryJob job = new(WebhookTestFactory.CreateEndpointId(), partitionKey, "retry.event", WebhookTestFactory.CreateEvent());
    ...
    public sealed class TheConcurrencyStressTests {
        ...
        public async Task EnqueueAsync_UnderHighConcurrency_PreservesStrictFifoSequence_PerPartition() {
            ...
            Task[] tasks = Enumerable.Range(0, partitionCount).Select(partitionIndex => {
                ...
                return Task.Run(async () => {
                    for(int sequenceId = 0; sequenceId < eventsPerPartition; sequenceId++) {
                        // Event payload stores the sequenceId
            ...
            // Assert: Drain each shard and verify that for any given partition, sequence numbers are strictly incrementing (FIFO)
            ...
            // All 10 partitions must have exactly 50 events in strict [0, 1, 2 ... 49] order!
    ...
    public sealed class TheGuardsAndCancellation {
        ...
        public async Task EnqueueAsync_ThrowsOperationCanceledException_WhenCancelledOnBoundedFullShard() {
            ...
            await sharded.EnqueueAsync(fillerJob); // fills the bounded capacity
