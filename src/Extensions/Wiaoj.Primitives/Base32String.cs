...
/// high-performance, allocation-optimized RFC 4648 encoder/decoder internally.
/// </remarks>
...
public readonly record struct Base32String : IEquatable<Base32String> {
    ...
    private static ReadOnlySpan<byte> DecodeTable => [
        ...
        0xFF, 0xFF, 26,   27,   28,   29,   30,   31,   // 48-55 ('0'-'7') -> '2'-'7' are 26-31. '0','1' are invalid in RFC4648 usually, but here mapped for safety check
        ...
        0xFF, 0,    1,    2,    3,    4,    5,    6,    // 96-103 ('a'-'g') - Case insensitive support
    ...
    public static Base32String FromBytes(ReadOnlySpan<byte> bytes) {
        ...
        int padding = (bytes.Length % 5) switch {
            0 => 0,
            1 => 6,
            2 => 4,
            3 => 3,
        ...
        int totalLength = charCount + padding;

        // Allocation-free string creation
        string encoded = string.Create(totalLength, bytes.ToArray(), (chars, input) => {
            ...
            while (bitIndex < inputBitLength) {
                ...
                if (bitOffset <= 3) {
                    // All 5 bits are in the current byte
        ...
        return new Base32String(encoded);
    }
    ...
    public static Base32String Parse(string s) {
        ...
        if (TryParse(s.AsSpan(), out Base32String result)) return result;
    ...
    public static Base32String Parse(ReadOnlySpan<char> s) {
        if (TryParse(s, out Base32String result)) return result;
    ...
    public static bool TryParse(ReadOnlySpan<char> s, out Base32String result) {
        ...
        // Validate characters and padding
        int paddingCount = 0;
        bool paddingStarted = false;

        // Optimization: Check basic structural validity without full decode
        for (int i = 0; i < s.Length; i++) {
            ...
            if (paddingStarted) {
                // Data character after padding is invalid
                result = default;
                return false;
            }
        ...
        // Length check (must be multiple of 8 for strict RFC 4648, though some impls relax this)
        if (s.Length % 8 != 0) {
            result = default;
            return false;
        }

        // Normalize to Uppercase if needed (RFC implies uppercase, but we allow parsing lower)
        // If string contains lowercase, we create a new one, otherwise use as is.
        string finalString = s.ToString().ToUpperInvariant();

        result = new Base32String(finalString);
        return true;
    }

    /// <inheritdoc cref="IUtf8SpanParsable{TSelf}.Parse(ReadOnlySpan{byte}, IFormatProvider?)"/>
    public static Base32String Parse(ReadOnlySpan<byte> utf8Text) {
        if (TryParse(utf8Text, out Base32String result)) return result;
        throw new FormatException("The input is not a valid Base32 string.");
    }
    ...
    public bool TryDecode(Span<byte> destination, out int bytesWritten) {
        ...
        for (int i = 0; i < unpaddedLength; i++) {
            char c = input[i];
            int val = DecodeTable[c]; // Already validated in TryParse
