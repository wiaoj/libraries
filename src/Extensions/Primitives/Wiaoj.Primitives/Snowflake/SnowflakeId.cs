...
[SkipLocalsInit]
[System.Diagnostics.DebuggerDisplay("{ToString()} (Raw: {Value})")]
public readonly struct SnowflakeId :
    ...
    private static ISnowflakeGenerator _sharedGenerator = SnowflakeGenerator.Default;
    ...
    private static void ConfigureInternal(ISnowflakeGenerator generator) {
        Preca.ThrowIfNull(generator);
        _configLock.Enter();
        _sharedGenerator = generator;
        _configLock.Exit();
    }
    ...
    /// <summary>
    /// Extracts the <see cref="UnixTimestamp"/> from a raw 64-bit Snowflake identifier using custom parameters.
    /// This utility is intended for decoding IDs generated by external systems or legacy configurations 
    /// that may differ from the current global settings.
    /// </summary>
    /// <param name="rawId">The raw 64-bit integer Snowflake ID to decode.</param>
    /// <param name="epoch">The custom <see cref="UnixTimestamp"/> (epoch) used as the start date by the source system.</param>
    /// <param name="timestampShift">
    /// The number of bits the timestamp was shifted to the left (typically the sum of NodeId and Sequence bits). 
    /// Defaults to 22.
    /// </param>
    /// <returns>A <see cref="UnixTimestamp"/> representing the exact moment the ID was generated.</returns>
    /// <remarks>
    /// <para>
    /// <b>Caution:</b> Different Snowflake implementations allocate different bit counts for Node IDs and Sequences. 
    /// Ensure the <paramref name="timestampShift"/> matches the source system's configuration to avoid incorrect date calculations.
    /// </para>
    /// <para>
    /// For IDs generated by the local system, prefer using the instance method <see cref="ToUnixTimestamp()"/> 
    /// which automatically uses the configured global generator settings.
    /// </para>
    /// </remarks>
    public static UnixTimestamp ExtractTimestamp(long rawId, UnixTimestamp epoch, int timestampShift = 22) {
        // Shifting right to isolate the timestamp delta bits
        long delta = rawId >> timestampShift;

        // Combining the delta with the provided epoch's total milliseconds
        return UnixTimestamp.FromMilliseconds(delta + epoch.TotalMilliseconds);
    }
    ...
    /// <summary>
    /// Creates a <see cref="SnowflakeId"/> from a <see cref="UnixTimestamp"/>.
    /// This generates a 'floor' ID (timestamp part set, others zero), 
    /// which is ideal for time-range database queries.
    /// </summary>
    /// <param name="timestamp">The timestamp to convert.</param>
    /// <returns>A SnowflakeId representing the start of that timestamp.</returns>
    public static SnowflakeId From(UnixTimestamp timestamp) { 
        return _sharedGenerator.CreateIdFromTimestamp(timestamp);
    }
