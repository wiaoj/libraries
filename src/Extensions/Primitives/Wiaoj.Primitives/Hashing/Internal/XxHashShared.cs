...
/// <summary>Shared implementation of the XXH3 hash algorithm for 64-bit in <see cref="XxHash3Core"/> and <see cref="XxHash128Core"/> version.</summary>
#if NET
[SkipLocalsInit]
#endif
internal static unsafe class XxHashShared {
    ...
    static XxHashShared() {
        ...
        ReadOnlySpan<ulong> defaultSecret3UInt64 = MemoryMarshal.Cast<byte, ulong>(DefaultSecret[3..]);
        ...
        fixed(byte* buffer = state.Buffer) {
            ...
            fixed(byte* sourcePtr = &MemoryMarshal.GetReference(source)) {
                ...
                if(state.BufferedCount != 0) {
                    ...
                    source[..loadSize].CopyTo(new Span<byte>(buffer + state.BufferedCount, loadSize));
                ...
                Span<byte> remaining = new(buffer, source.Length - sourceIndex);
                Debug.Assert(sourceIndex < source.Length);
                Debug.Assert(remaining.Length <= InternalBufferLengthBytes);
                Debug.Assert(state.BufferedCount == 0);
                source[sourceIndex..].CopyTo(remaining);
