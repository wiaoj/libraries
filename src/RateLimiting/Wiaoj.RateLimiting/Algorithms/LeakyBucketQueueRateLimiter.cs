...
/// actually <b>waits</b> until the requests ahead of it have drained before returning <see cref="RateLimitDecision.Allowed"/>.
...
public sealed class LeakyBucketQueueRateLimiter : IRateLimitAlgorithm {
    ...
    /// </summary>
    /// <param name="capacity">The maximum backlog a key can accumulate before being rejected. Must be greater than zero.</param>
    /// <param name="period">The time it takes a fully backed-up queue to drain to empty. Must be greater than <see cref="TimeSpan.Zero"/>.</param>
    ...
    /// <summary>
    /// Creates a new leaky-bucket (queue/shaping) rate limiter with a custom time provider.
    /// </summary>
    /// <param name="capacity">The maximum backlog a key can accumulate before being rejected. Must be greater than zero.</param>
    /// <param name="period">The time it takes a fully backed-up queue to drain to empty. Must be greater than <see cref="TimeSpan.Zero"/>.</param>
    /// <param name="timeProvider">The time provider driving scheduling math and delay operations. Defaults to <see cref="TimeProvider.System"/> when omitted.</param>
    public LeakyBucketQueueRateLimiter(
        int capacity,
        TimeSpan period,
    ...
    /// <summary>
    /// Creates a new leaky-bucket (queue/shaping) rate limiter with custom time provider and diagnostic logging.
    /// </summary>
    /// <param name="capacity">The maximum backlog a key can accumulate before being rejected. Must be greater than zero.</param>
    /// <param name="period">The time it takes a fully backed-up queue to drain to empty. Must be greater than <see cref="TimeSpan.Zero"/>.</param>
    /// <param name="timeProvider">The time provider driving scheduling math and delay operations. Defaults to <see cref="TimeProvider.System"/> when omitted.</param>
    ...
    private async Task<RateLimitDecision> WaitForTurnAndCompleteAsync(string key,
                                                                      int cost,
                                                                      long incrementTicks,
                                                                      TimeSpan wait,
        ...
        // Record the actual elapsed wait duration and emit completion telemetry now that the delay succeeded.
        ...
        // Read the latest live state for this key so remaining capacity accounts for any requests queued behind this one.
