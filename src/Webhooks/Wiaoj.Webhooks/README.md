...
- **Enterprise SSRF Protection:** Defends against Server-Side Request Forgery with `Wiaoj.Net`'s `OutboundNetworkPolicy`, inspecting resolved IP addresses at the TCP socket layer (blocking RFC 1918 private ranges, loopback, cloud metadata `169.254.169.254`, and encapsulated 6to4, NAT64, and Teredo tunneling attacks). An internal network can be allowed as an exception with `UseNetworkPolicy(OutboundNetworkPolicy.PublicOnly with { AllowedNetworks = [...] })`, and `WebhookEndpointBuilder.TryBuildAsync` reports a refused target URL as a result instead of an exception.
...
**SSRF refusals at two levels.** Subscribe to the `Wiaoj.Net` meter as well to see refusals as `wiaoj.net.outbound.refused`, tagged with a `stage`, a `reason` and an address `scope`. The two counters show the same event at different levels, so don't add them up:

| Situation | `wiaoj.webhooks.ssrf.blocked.count` | `wiaoj.net.outbound.refused` |
| --- | --- | --- |
| Delivery refused, no proxy | +1 (the delivery) | +1, `stage=connect` |
| Delivery refused through a proxy (checked before the request is sent) | +1 | +1, `stage=request` |
| Another `HttpClient` in the application with a policy refuses a destination | nothing | +1 |
