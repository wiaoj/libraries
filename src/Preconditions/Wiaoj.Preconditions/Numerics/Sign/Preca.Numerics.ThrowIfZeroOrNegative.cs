...
public static partial class Preca {
    /// <summary>
    /// Validates that the specified numeric value is greater than zero.
    /// </summary>
    ...
    /// <exception cref="PrecaArgumentOutOfRangeException">Thrown when <paramref name="argument"/> is zero or negative. Inherits from <see cref="ArgumentOutOfRangeException"/>.</exception>
    /// <remarks>
    /// <para>This method ensures numeric values are strictly positive, rejecting both zero and negative values.</para>
    /// <para><strong>Usage:</strong> <c>Preca.ThrowIfZeroOrNegative(count);</c> - Ensures positive value</para>
    ...
    public static void ThrowIfNegativeOrZero<T>(T argument,
                                                [CallerArgumentExpression(nameof(argument))] string? paramName = null)
        where T : ISignedNumber<T> {
        Preca.ThrowIfNull(argument, paramName);

        // Fix: Handle IEEE 754 special cases correctly
        // - Both positive and negative zero should be treated as zero (invalid)
        // - NaN should be treated as neither positive nor negative (invalid)
        if (T.IsZero(argument) || (T.IsNegative(argument) && !T.IsZero(argument)) || T.IsNaN(argument)) {
            Thrower.ThrowPrecaArgumentOutOfRangeException(paramName, argument, PrecaMessages.Numeric.ValueCannotBeZeroOrNegative);
        }
    }

    /// <summary>
    /// Validates that the specified numeric value is greater than zero, using a custom exception factory.
    /// </summary>
    /// <typeparam name="T">The numeric type to validate. Must implement <see cref="ISignedNumber{T}"/>.</typeparam>
    ...
    /// <exception cref="Exception">Thrown when <paramref name="argument"/> is zero or negative, using the exception from <paramref name="exceptionFactory"/>.</exception>
    /// <remarks>
    /// <para>This overload enables domain-specific exception handling for zero or negative value validation.</para>
    /// <para><strong>Usage:</strong> <c>Preca.ThrowIfZeroOrNegative(count, () => new ArgumentException("Count must be positive"));</c></para>
    /// <para><strong>Related:</strong> Use when zero or negative validation failures require specific business exception types</para>
    /// </remarks>
    [DebuggerStepThrough, StackTraceHidden]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void ThrowIfNegativeOrZero<T, TException>(T argument,
    ...
    /// <summary>
    /// Validates that the specified numeric value is greater than zero, using a stateful custom exception factory.
    /// </summary>
    /// <typeparam name="T">The numeric type to validate. Must implement <see cref="ISignedNumber{T}"/>.</typeparam>
    ...
    /// <summary>
    /// Validates that the specified numeric value is greater than zero, throwing a specific exception type.
    /// </summary>
    /// <typeparam name="T">The numeric type to validate. Must implement <see cref="ISignedNumber{T}"/>.</typeparam>
    /// <typeparam name="TException">The type of exception to throw. Must have a parameterless constructor.</typeparam>
    /// <param name="argument">The numeric value to validate.</param>
    /// <param name="paramName">The name of the parameter being validated. This parameter is automatically populated by the compiler.</param>
    /// <exception cref="Exception">Thrown when <paramref name="argument"/> is zero or negative. The specific exception type is determined by the TException generic parameter.</exception>
    /// <remarks>
    /// <para>This overload enables throwing specific exception types while maintaining parameter name information.</para>
    /// <para><strong>Usage:</strong> <c>Preca.ThrowIfZeroOrNegative&lt;int, InvalidOperationException&gt;(count);</c></para>
