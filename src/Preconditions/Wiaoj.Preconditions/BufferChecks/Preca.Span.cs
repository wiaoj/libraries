...
public static partial class Preca {
    /// <summary>
    /// Validates that the specified Span is not empty.
    /// </summary>
    ...
    /// <remarks>
    /// Use this method to ensure span parameters contain meaningful content.
    /// Empty spans indicate invalid buffer states for most operations.
    /// </remarks>
    [DebuggerStepThrough, StackTraceHidden]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void ThrowIfEmpty<T>(in Span<T> span,
                                       [CallerArgumentExpression(nameof(span))] string? paramName = null) {
        if (span.IsEmpty) {
            Thrower.ThrowPrecaArgumentException(PrecaMessages.Buffer.SpanCannotBeEmpty, paramName);
        }
    }

    /// <summary>
    /// Validates that the specified Span is not empty, using a type-safe custom exception factory.
    /// </summary>
    /// <typeparam name="T">The type of elements in the span.</typeparam>
    ...
    /// <remarks>
    /// Use this overload when you need type-safe custom exception types for span validation.
    /// The factory is only invoked if validation fails, ensuring no performance overhead in success cases.
    /// </remarks>
    [DebuggerStepThrough, StackTraceHidden]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void ThrowIfEmpty<T, TException>(in Span<T> span, [NotNull] Func<TException> exceptionFactory)
    ...
    /// <summary>
    /// Validates that the specified Span is not empty, throwing a specific exception type.
    /// </summary>
    /// <typeparam name="T">The type of elements in the span.</typeparam>
    ...
    [DebuggerStepThrough, StackTraceHidden]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void ThrowIfEmpty<T, TException>(in Span<T> span,
                                                   [CallerArgumentExpression(nameof(span))] string? paramName = null)
